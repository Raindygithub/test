<template>
      ...
      <div class="time-b w100">
        ...
        <div class="time-controller">
          <el-button-group>
            ...
            ></el-button>
            <el-button round class="date-btn">第{{count}}周</el-button>
            <el-button
              type="primary"
              icon="el-icon-arrow-right"
              @click="changeCount(1), getWeek(count)"
              :disabled="count===weekCourse.length?true:false"
            ></el-button>
          </el-button-group>
        </div>
      </div>
  ...
  export default {
    ...
    created() {
         axios.get('/stu/class',{params:{s_no:this.s_no,count:this.count,}}).then(res=>{
            let result = res.data.code;
            let data=res.data.data;
            if(result){
                console.log("data:",data[0]);
                console.log(typeof(data[0]));
                //this.weekCourse.length=0;
                let week=[];
                for(let i=0;i<7;i++){
                    week.push(data[i]);
                }
      ...
      this.init();
      this.getWeek(0);
        console.log("data:",typeof(this.weekCourse));
    },
    methods: {
      //改变选择器次数
      changeCount(i) {
      ...
      // 排序周期和课数
      sortData() {
        console.log("week",this.weekCourse);
        //周期
